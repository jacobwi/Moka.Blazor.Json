...
public sealed partial class MokaJsonSearchOverlay : ComponentBase
{
    ...
    {
        await OnQueryChanged.InvokeAsync(e.Value?.ToString() ?? "");
    }
    ...
    {
        await OnCaseSensitiveChanged.InvokeAsync(bool.TryParse(e.Value?.ToString(), out var v) && v);
    }

    private async Task HandleRegexChange(ChangeEventArgs e)
    {
        await OnRegexChanged.InvokeAsync(bool.TryParse(e.Value?.ToString(), out var v) && v);
    }
