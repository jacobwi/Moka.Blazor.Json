...
public sealed partial class MokaJsonViewer : ComponentBase, IMokaJsonViewer, IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (_documentManager is not null) await _documentManager.DisposeAsync();

        if (_contextMenu is not null) await _contextMenu.DisposeAsync();
    }
        ...
        try
        {
            ...
            else
            {
                ...
                _ = Task.Run(() =>
                {
                    ...
                    var nc = manager.CountNodes();
                    var md = manager.GetMaxDepth();
                    if (cts.Token.IsCancellationRequested) return;
                    _ = InvokeAsync(() =>
                    {
                        if (cts.Token.IsCancellationRequested) return;
    ...
    {
        if (!_isLoaded || _documentManager is null) return;

        if (_scopedPath is not null)
            try
            {
                var scopedElement = _documentManager.NavigateToElement(_scopedPath);
    ...
    private async Task HandleCopyAll()
    {
        ...
        var json = _documentManager.GetJsonString(_isFormatted);
    ...
    private void HandleSearchQueryChanged(string query)
    {
        ...
        var debounceMs = OptionsAccessor.Value.SearchDebounceMs;
        if (debounceMs <= 0)
        {
            ExecuteSearch();
            return;
    ...
    {
        var path = _searchEngine.NextMatch();
        if (path is not null)
        {
    ...
    {
        var path = _searchEngine.PreviousMatch();
        if (path is not null)
        {
            EnsurePathExpanded(path);
            _treeFlattener.SetSearchMatches(_searchEngine.MatchPaths, _searchEngine.ActiveMatchPath);
    ...
    private void ExecuteSearch()
    {
        ...
        if (_searchEngine.ActiveMatchPath is not null) EnsurePathExpanded(_searchEngine.ActiveMatchPath);
    ...
    private async Task HandleSelect(string path)
    {
        ...
        if (_documentManager is not null)
            try
            {
                var element = _documentManager.NavigateToElement(path);
                var segments = path.Split('/', StringSplitOptions.RemoveEmptyEntries);
                _selectedDepth = segments.Length;
                var propName = segments.Length > 0 ? segments[^1].Replace("~1", "/").Replace("~0", "~") : null;
    ...
    private async Task HandleContextMenuRequest((string Path, double ClientX, double ClientY) args)
    {
        ...
        try
        {
            var element = _documentManager.NavigateToElement(args.Path);
            var segments = args.Path.Split('/', StringSplitOptions.RemoveEmptyEntries);
            var propName = segments.Length > 0 ? segments[^1].Replace("~1", "/").Replace("~0", "~") : null;
            ...
            if (_contextMenu is not null) await _contextMenu.ShowAsync(args.ClientX, args.ClientY);
        }
        catch (KeyNotFoundException)
        {
        }
    ...
    private async Task HandleKeyDown(KeyboardEventArgs e)
    {
        if (e is { CtrlKey: true, Key: "f" })
            ...
            Undo();
        else if (e is { CtrlKey: true, Key: "y" } && !ReadOnly) Redo();
    }
    ...
    private void BuildContextActions()
    {
        ...
        if (ContextMenuActions is not null) _allContextActions.AddRange(ContextMenuActions);
    }
    ...
    {
        if (_documentManager is null) return newSubtreeJson;
        ...
        var rootElement = _documentManager.RootElement;
        ...
        JsonElement replacement)
    {
        ...
        var targetSegment = pathSegments[segmentIndex].Replace("~1", "/").Replace("~0", "~");
    ...
    private void CollapseSubtree(string path)
    {
        ...
        foreach (var p in toRemove) _treeFlattener.Collapse(p);
    ...
    private void EnsurePathExpanded(string path)
    {
        ...
        foreach (var segment in segments)
        {
            ...
            if (currentPath != path) _treeFlattener.Expand(currentPath);
        }
    }
    ...
    {
        if (_documentManager is null) return 0;
    ...
    {
        return _documentManager?.GetJsonString(indented) ?? string.Empty;
    }
