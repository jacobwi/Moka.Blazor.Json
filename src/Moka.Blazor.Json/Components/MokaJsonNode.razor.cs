...
public sealed partial class MokaJsonNode : ComponentBase
{
    ...
    private string CssClass
    {
        get
        {
            var css = "moka-json-node";
            if (IsSelected) css += " moka-json-node--selected";
            if (Node.IsSearchMatch) css += " moka-json-node--search-match";
            if (Node.IsActiveSearchMatch) css += " moka-json-node--search-active";
            return css;
        }
    }
    ...
    {
        if (Node.ChildCount == 0) return Node.ValueKind == JsonValueKind.Object ? "" : "";

        var itemWord = Node.ChildCount == 1 ? "item" : "items";
    ...
    {
        if (!Node.IsClosingBracket) await OnSelect.InvokeAsync(Node.Path);
    }

    private async Task HandleContextMenu(MouseEventArgs e)
    {
        if (!Node.IsClosingBracket) await OnContextMenu.InvokeAsync((Node.Path, e.ClientX, e.ClientY));
    }

    private async Task HandleDoubleClick(MouseEventArgs _)
    {
        if (!Node.IsClosingBracket) await OnDoubleClick.InvokeAsync(Node.Path);
    }
