...
internal sealed class JsonSearchEngine
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_matchPaths.Count > 0) ActiveMatchIndex = 0;
    ...
    {
        if (_matchPaths.Count == 0) return null;
    ...
    private void SearchPlainTextValue(
        ...
        CancellationToken cancellationToken)
    {
        if (!options.SearchValues) return;
        if (element.ValueKind is JsonValueKind.Object or JsonValueKind.Array) return;

        var valueText = element.ValueKind == JsonValueKind.String
            ? element.GetString() ?? ""
            : element.GetRawText();

        if (valueText.Contains(query, comparison) && _matchPathSet.Add(path)) _matchPaths.Add(path);
    }
        ...
        switch (element.ValueKind)
        {
            case JsonValueKind.Object:
                foreach (var prop in element.EnumerateObject())
                {
                    ...
                    if (options.SearchKeys && regex.IsMatch(prop.Name)) _matchPaths.Add(childPath);

                    if (options.SearchValues &&
                        prop.Value.ValueKind is not (JsonValueKind.Object or JsonValueKind.Array))
                    {
                        var valueText = prop.Value.ValueKind == JsonValueKind.String
                            ? prop.Value.GetString() ?? ""
                            : prop.Value.GetRawText();

                        if (regex.IsMatch(valueText) && _matchPathSet.Add(childPath)) _matchPaths.Add(childPath);
                    }
            ...
            case JsonValueKind.Array:
                ...
                foreach (var item in element.EnumerateArray())
                {
                    ...
                    {
                        var valueText = item.ValueKind == JsonValueKind.String
                            ? item.GetString() ?? ""
                            : item.GetRawText();

                        if (regex.IsMatch(valueText)) _matchPaths.Add(childPath);
                    }
